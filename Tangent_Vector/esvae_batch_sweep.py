#!/usr/bin/env python
"""Phase 1c: batch-size sweep on the winning (R, hidden, beta_kl) + best epoch.

Reads:
  - results/esvae_clf_config.json (winner from main sweep)
  - results/esvae_epoch_sweep.csv (best epoch chosen by macro-F1)
Writes:
  - results/esvae_batch_sweep.csv
"""
...
def main():
    ...
    ap.add_argument("--epoch-csv", default=str(REPO_ROOT / "Tangent_Vector" / "results" / "esvae_epoch_sweep.csv"))
    ...
    epoch_df = pd.read_csv(args.epoch_csv)
    best_row = epoch_df.loc[epoch_df["pooled_macroF1"].idxmax()]
    best_epochs = int(best_row["epochs"])
    print(f"Winner enc={enc}  knn={knn_cfg}  best epochs from epoch sweep={best_epochs} (F1={best_row['pooled_macroF1']:.4f})")
    ...
    for bs in BATCH_GRID:
        ...
        for k, test_subjects in enumerate(folds):
            ...
            model = train_esvae_fold(
                ...
                R=enc["R"], num_epochs=best_epochs, lr=enc["lr"],
                batch_size=min(bs, len(tr_idx)),
        ...
        print(f"batch_size={bs:4d}  acc={acc:.4f}  macroF1={f1m:.4f}", flush=True)
        rows.append({"batch_size": bs, "epochs": best_epochs, "pooled_acc": acc, "pooled_macroF1": f1m})

    out = Path(args.output_dir) / "esvae_batch_sweep.csv"
    pd.DataFrame(rows).to_csv(out, index=False)
    print(f"\nSaved {out}")
    best = max(rows, key=lambda r: r["pooled_macroF1"])
    print(f"Best batch_size={best['batch_size']}  epochs={best['epochs']}  macroF1={best['pooled_macroF1']:.4f}")

    # Persist the final fully-tuned encoder config so downstream scripts pick it up.
    final_cfg = {
        "encoder": {**enc, "epochs": best["epochs"], "batch_size": best["batch_size"]},
        "knn": knn_cfg,
        "pooled_macroF1": best["pooled_macroF1"],
    }
    final_path = Path(args.output_dir) / "esvae_clf_config.json"
    with open(final_path, "w") as fh:
        json.dump(final_cfg, fh, indent=2)
    print(f"Updated {final_path} with final tuned config.")
