...
Sparse-ST-GCN (CVPR 2025) to this same 400-sample top10 subset. For the
tangent side, the adapted runner reads `../aligned_data/tangent_vecs100.pkl`
directly and keeps the same **8 L5SO subject folds** as `cv_utils.py`.

Under a pragmatic `20`-epoch single-stream subject-CV benchmark:

| Official model | Tangent Top-1 | Tangent Macro-F1 | 95% CI |
|---|---:|---:|---:|
| ProtoGCN | 0.5525 | **0.5510** | [0.5024, 0.5941] |
| Sparse-ST-GCN | 0.5150 | 0.5006 | [0.4595, 0.5421] |

Compared against the corresponding raw-input runs:

| Official model | Tangent | Raw | Δ (TV - RS) |
|---|---:|---:|---:|
| ProtoGCN | 0.5510 | 0.6167 | **-0.0656** |
| Sparse-ST-GCN | 0.5006 | 0.4893 | **+0.0113** |

So the tangent edge still appears for Sparse-ST-GCN, but ProtoGCN
favours raw coordinates on this tiny subset at the 20-epoch budget.
That does **not** invalidate the ES-VAE result above; it just means the
representation effect is architecture- and training-budget-dependent
once we move to newer NTU-specialised models.
