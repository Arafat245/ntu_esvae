...
action classification on RAW NTU skeletons (linearly resampled to T=100).
Subject-level leave-5-subjects-out CV — identical partition to
Tangent_Vector for matched-pair comparison.

Each sample is a single sequence of length T=100 with 75 channels
(= 25 joints x 3 coords). No sliding window. Per-channel z-score on
the training fold standardises the metric coordinates.
...
def main():
    ...
    ap.add_argument("--T", type=int, default=100,
                    help="Common temporal length after linear resampling.")
    ...
    _, X, _, y, subj, _ = load_data(args.T)
    # X already shaped (N, 75, T) by load_data
